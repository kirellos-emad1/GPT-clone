...
import { updateChat } from "@/app/actions/update-chat";
...
const ChatSettingsMenu = () => {
    ...
    const sendToServer = useCallback(
        async (value: string, id: string) => {
            try {
                await updateChat(id, { system_prompt: value });
            } catch (error) {
    ...
    const resetSystemInput = useCallback(async () => {
        ...
        try {
            await updateChat(currentChat.id, { system_prompt: defaultSystemPrompt });
        } catch (error) {
            ...
            <Dialog>
                <DialogTrigger className="flex items-center max-w-full gap-4 pt-2 pb-4 overflow-x-auto text-xs whitespace-nowrap">
                    ...
                    <div className="px-2 py-1 bg-white rounded-md shadow-sm dark:bg-neutral-900">
                        ...
                        {currentChat?.system_prompt === defaultSystemPrompt
                            ? "Default"
                            : "Custom"}
                    </div>
                    <div className="flex items-center px-2 py-1 bg-white rounded-md shadow-sm dark:bg-neutral-900">
                        ...
                        <div
                            className={`w-2 h-2 mx-1 rounded-full ${tokenSizeLimitExceeded ? "bg-red-400" : "bg-emerald-400"
                                }`}
                        />
                    ...
                    <div className="mt-6">
                        ...
                        <Select
                            onValueChange={async (value: "gpt-4" | "gpt-3.5-turbo") => {
                                ...
                                try {
                                    await updateChat(currentChat.id, { model: value });
                                } catch (error) {
                        ...
                        <div className="mt-6">
                            ...
                            <Select
                                onValueChange={async (value: "global" | "chat") => {
                                    ...
                                    try {
                                        await updateChat(currentChat.id, { history_type: value });
                                    } catch (error) {
                                        console.error('Failed to update history type:', error);
                                        // Handle error appropriately
                                    }
                                }}
                                value={currentChat?.history_type?? 'chat'}
                            >
                                <SelectTrigger className="w-full mt-3">
                                    <SelectValue placeholder="Select a model." />
                                </SelectTrigger>
                        ...
                        <div className="mt-6">
                            ...
                            <TextareaDefault
                                ...
                                onChange={async (e)  => {
                                    ...
                                    await debouncedSendToServer(
                                        value ? value : defaultSystemPrompt,
