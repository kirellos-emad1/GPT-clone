...
const openAIPayload = atom<OpenAIStreamPayload>((get) => {
    ...
    if (isContextNeeded) {
        ...
        let remainingTokenSizeForCurrentChat =
            tokenSizeLimitState.remainingTokenForCurrentChat;
...
export const tokenSizeLimitAtom = atom((get) => {
    ...
    const total =
        limit - systemPromptTokenSize - buffer - responseLimit - contextTokenSize;

    return {
        remainingToken: total - get(tokenCountAtom).currentChatToken,
        remainingTokenForCurrentChat: total,
